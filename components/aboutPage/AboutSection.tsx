...
function MilestoneCard({ m, align }: { m: typeof milestones[0]; align: 'left' | 'right' }) {
  return (
    <div style={{
      ...
      width: '70%',
      boxShadow: m.done ? '0 4px 20px rgba(68,178,76,0.08)' : '0 2px 10px rgba(0,0,0,0.04)',
      ...
      {m.done && (
        <div style={{
          ...
          background: GREEN, borderRadius: 100, padding: '2px 10px',
          fontSize: 10, fontWeight: 700, color: '#fff',
          fontFamily: "'Inter', sans-serif", letterSpacing: '0.08em', textTransform: 'uppercase',
        }}>✓ Done</div>
      )}
      <p style={{ fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: 17, color: BLUE_DARK, marginBottom: 3 }}>{m.title}</p>
      <p style={{ fontSize: 13, color: '#64748b', fontFamily: "'Inter', sans-serif", lineHeight: 1.5 }}>{m.text}</p>
    </div>
  );
}
...
function YearPill({ year, done }: { year: string; done: boolean }) {
  return (
    <div style={{
      ...
      background: done ? `linear-gradient(135deg,${GREEN},${GREEN_DK})` : 'rgba(16,84,156,0.06)',
      border: done ? 'none' : '1.5px dashed #cbd5e1',
      fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: 16,
      color: done ? '#fff' : BLUE,
...
function DesktopMilestoneRow({ m, i }: { m: typeof milestones[0]; i: number }) {
  const isLeft = i % 2 === 0;
  return (
    <motion.div
      ...
      style={{ display: 'grid', gridTemplateColumns: '1fr 48px 1fr', alignItems: 'center', marginBottom: 8 }}
    >
...
// ── Mobile timeline row ───────────────────────────────────────────────────────
function MobileMilestoneRow({ m, i }: { m: typeof milestones[0]; i: number }) {
  return (
    <motion.div
      ...
      style={{ display: 'grid', gridTemplateColumns: '32px 1fr', alignItems: 'flex-start', gap: 0, marginBottom: 16 }}
    >
      {/* Dot */}
  ...
  return (
    <main style={{ background: '#fff', minHeight: '100vh', paddingTop: 80 }}>
      ...
      <section style={{ position: 'relative', minHeight: '72vh', display: 'flex', alignItems: 'center', overflow: 'hidden', paddingBottom: 48 }}>
        ...
        <div style={{ position: 'absolute', top: 0, right: 0, zIndex: 1, fontFamily: "'Fraunces', serif", fontSize: 'clamp(4rem,14vw,14rem)', fontWeight: 900, lineHeight: 0.85, letterSpacing: '-0.05em', color: 'rgba(255,255,255,0.04)', pointerEvents: 'none', userSelect: 'none' }}>ABO<br />UT</div>

        <div style={{ position: 'relative', zIndex: 2, maxWidth: 1280, margin: '0 auto', width: '100%', padding: `100px ${px}px 80px` }}>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            ...
            <span style={{ color: '#7dd880', fontSize: 12, fontFamily: "'Inter', sans-serif", fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Since 2019 · Raipur, India</span>
          </motion.div>

          <motion.h1 initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.1, ease: EASE }}
            style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(2rem,6vw,5rem)', color: '#fff', letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 20, maxWidth: 680 }}>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.22, ease: EASE }}
            style={{ color: 'rgba(255,255,255,0.65)', fontSize: isMobile ? 15 : 17, fontFamily: "'Inter', sans-serif", lineHeight: 1.75, maxWidth: 520, marginBottom: 36 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.34, ease: EASE }}
            ...
            {[{ val: '2019', lbl: 'Founded' }, { val: '500+', lbl: 'Stores by 2029' }, { val: '24×7', lbl: 'Operations' }, { val: 'FOCO', lbl: 'Business Model' }].map(s => (
              <div key={s.lbl}>
                <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.3rem,3vw,2.2rem)', color: GREEN, lineHeight: 1 }}>{s.val}</div>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.38)', marginTop: 4 }}>{s.lbl}</div>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ── INTRODUCTION ── */}
      <section style={{ padding: `72px ${px}px`, maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: isDesktop ? 64 : 40, alignItems: 'center' }}>
          <motion.div initial={{ opacity: 0, x: isDesktop ? -32 : 0, y: isDesktop ? 0 : 20 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: EASE }}>
            <p style={{ color: GREEN, fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif", marginBottom: 12 }}>Who We Are</p>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.6rem,3.5vw,2.8rem)', color: BLUE_DARK, letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: 20 }}>
              Laundry isn't just about cleaning clothes — it's about care, convenience & trust.
            </h2>
            <p style={{ color: '#475569', fontSize: 15, lineHeight: 1.85, fontFamily: "'Inter', sans-serif", marginBottom: 16 }}>
            ...
            <p style={{ color: '#475569', fontSize: 15, lineHeight: 1.85, fontFamily: "'Inter', sans-serif" }}>
          ...
          <motion.div initial={{ opacity: 0, x: isDesktop ? 32 : 0, y: isDesktop ? 0 : 20 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: EASE, delay: 0.15 }}
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {marketStats.map(({ val, label }, i) => (
              <div key={label} style={{
                background: i === 0 ? `radial-gradient(circle at top right, rgba(68,178,76,0.12) 0%, transparent 65%), radial-gradient(circle at bottom left, rgba(16,84,156,0.3) 0%, transparent 65%), linear-gradient(145deg, #10549c 0%, #0a3d75 45%, #072d57 100%)` : CREAM,
                borderRadius: 20, padding: isMobile ? '20px 14px' : '28px 20px',
                textAlign: 'center', border: i === 0 ? 'none' : '1.5px solid #e8edf5',
              }}>
                <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: isMobile ? 22 : 28, letterSpacing: '-0.02em', marginBottom: 6, color: i === 0 ? GREEN : BLUE }}>{val}</div>
                <div style={{ fontSize: 12, lineHeight: 1.5, color: i === 0 ? 'rgba(255,255,255,0.6)' : '#64748b', fontFamily: "'Inter', sans-serif" }}>{label}</div>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ── MISSION & VISION ── */}
      <section style={{ background: CREAM, padding: `72px ${px}px` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
            style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ color: GREEN, fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif", marginBottom: 10 }}>Purpose & Direction</p>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.6rem,3.5vw,2.8rem)', color: BLUE_DARK, letterSpacing: '-0.03em' }}>Mission & Vision</h2>
          </motion.div>

          <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: 28 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease: EASE }}
              ...
              <div style={{ height: 300, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                ...
                <div style={{ position: 'absolute', top: 16, left: 16, display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(68,178,76,0.22)', border: '1px solid rgba(68,178,76,0.45)', borderRadius: 100, padding: '5px 12px' }}>
                  ...
                  <span style={{ color: '#7dd880', fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', fontFamily: "'Inter', sans-serif" }}>PURPOSE</span>
                </div>
                <div style={{ position: 'absolute', bottom: 22, left: 22, display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Our Mission</p>
                    <h3 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: '1.5rem', color: '#fff', letterSpacing: '-0.02em', lineHeight: 1 }}>Why We Exist</h3>
                  </div>
                </div>
              </div>
              <div style={{ background: `radial-gradient(circle at top right, rgba(68,178,76,0.12) 0%, transparent 65%), radial-gradient(circle at bottom left, rgba(16,84,156,0.3) 0%, transparent 65%), linear-gradient(145deg, #10549c 0%, #0a3d75 45%, #072d57 100%)`, padding: '26px 22px 28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <p style={{ color: 'rgba(255,255,255,0.78)', fontSize: 15, lineHeight: 1.85, fontFamily: "'Inter', sans-serif" }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                  {['Tech-driven operations at every step', 'Home-curated, doorstep-delivered service', 'Setting new industry benchmarks in quality'].map(text => (
                    <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 11, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 11, padding: '10px 13px' }}>
                      ...
                      <span style={{ color: 'rgba(255,255,255,0.84)', fontSize: 13, lineHeight: 1.5, fontFamily: "'Inter', sans-serif" }}>{text}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(68,178,76,0.15)', border: '1px solid rgba(68,178,76,0.3)', borderRadius: 100, padding: '8px 16px', alignSelf: 'flex-start', marginTop: 'auto' }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: GREEN, display: 'inline-block' }} />
                  <span style={{ color: '#7dd880', fontSize: 13, fontFamily: "'Inter', sans-serif", fontWeight: 600 }}>On a mission since 2019</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease: EASE, delay: 0.12 }}
              ...
              <div style={{ height: 300, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                <img src={VISION_IMG.src} alt="Vision" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', display: 'block' }} />
                ...
                <div style={{ position: 'absolute', top: 16, left: 16, display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.14)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 100, padding: '5px 12px', backdropFilter: 'blur(8px)' }}>
                  <span style={{ color: '#fff', fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', fontFamily: "'Inter', sans-serif" }}>🚀 AMBITION 2029</span>
                </div>
                <div style={{ position: 'absolute', bottom: 22, left: 22, display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Our Vision</p>
                    <h3 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: '1.5rem', color: '#fff', letterSpacing: '-0.02em', lineHeight: 1 }}>Where We're Going</h3>
                  </div>
                </div>
              </div>
              <div style={{ background: '#fff', padding: '26px 22px 28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <p style={{ color: '#64748b', fontSize: 13, fontFamily: "'Inter', sans-serif", lineHeight: 1.6, fontStyle: 'italic' }}>Where Prime Laundry is headed by 2029.</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                  {visionPoints.map((pt, i) => (
                    <motion.div key={pt}
                      ...
                      <div>
                        <p style={{ fontSize: 14, fontWeight: i === 2 ? 700 : 500, color: i === 2 ? '#1a3a1a' : '#334155', fontFamily: "'Inter', sans-serif", lineHeight: 1.5, margin: 0 }}>{pt}</p>
                        {i === 2 && <p style={{ fontSize: 11, color: GREEN, fontFamily: "'Inter', sans-serif", fontWeight: 600, marginTop: 3 }}>Target Year · 2029</p>}
                      </div>
                    </motion.div>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 'auto' }}>
                  {[{ label: '500+', sub: 'Stores' }, { label: '5K+', sub: 'Jobs' }, { label: '500', sub: 'Entrepreneurs' }].map(s => (
                    <div key={s.label} style={{ flex: 1, minWidth: 80, background: CREAM, border: '1.5px solid #e8edf5', borderRadius: 14, padding: '12px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, color: '#94a3b8', marginTop: 4, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{s.sub}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── TIMELINE ── */}
      <section style={{ padding: `80px ${px}px`, background: '#fff' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            style={{ textAlign: 'center', marginBottom: 64 }}>
            <p style={{ color: GREEN, fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif", marginBottom: 10 }}>Our Journey</p>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.6rem,3.5vw,2.8rem)', color: BLUE_DARK, letterSpacing: '-0.03em' }}>Built for the Long Run</h2>
          </motion.div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {milestones.map((m, i) =>
                isMobile
                  ? <MobileMilestoneRow key={m.year} m={m} i={i} />
                  : <DesktopMilestoneRow key={m.year} m={m} i={i} />
              )}
            </div>
          </div>
        </div>
      ...
      {/* ── WHY MODERN LAUNDRY ── */}
      <section style={{ background: CREAM, padding: `80px ${px}px` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            style={{ textAlign: 'center', marginBottom: 56 }}>
            <p style={{ color: GREEN, fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif", marginBottom: 10 }}>The Opportunity</p>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.6rem,3.5vw,2.8rem)', color: BLUE_DARK, letterSpacing: '-0.03em', marginBottom: 12 }}>Why Modern Laundry Services Matter</h2>
            <p style={{ color: '#64748b', fontSize: 15, fontFamily: "'Inter', sans-serif", lineHeight: 1.7, maxWidth: 520, margin: '0 auto' }}>India's laundry industry is at an inflection point. Here's what's driving the revolution.</p>
          </motion.div>

          <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? 'repeat(3, 1fr)' : isTablet ? 'repeat(2, 1fr)' : '1fr', gap: 24 }}>
              ...
              <motion.div key={title}
                ...
                <div style={{ padding: '20px 20px 24px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(68,178,76,0.1)', border: '1px solid rgba(68,178,76,0.25)', borderRadius: 100, padding: '4px 12px', width: 'fit-content' }}>
                    <span style={{ color: GREEN_DK, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif" }}>{tag}</span>
                  </div>
                  <h3 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: '1.15rem', color: BLUE_DARK, letterSpacing: '-0.01em', lineHeight: 1.25 }}>{title}</h3>
                  <p style={{ color: '#64748b', fontSize: 13.5, lineHeight: 1.8, fontFamily: "'Inter', sans-serif", flex: 1 }}>{desc}</p>
                  <div style={{ height: 1, background: '#f0f4f8', margin: '4px 0' }} />
                  <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 600, fontFamily: "'Inter', sans-serif" }}>{footer}</span>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section style={{
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ position: 'relative', zIndex: 2 }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 900, fontSize: 'clamp(1.6rem,3.5vw,2.8rem)', color: '#fff', letterSpacing: '-0.03em', marginBottom: 14 }}>
            Join Prime Laundry.<br />Become an Entrepreneur.
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.6)', fontFamily: "'Inter', sans-serif", fontSize: 16, margin: '0 auto 32px', maxWidth: 460 }}>Your clean start to a profitable future.</p>
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <motion.a href="/supportPage" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.96 }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 28px', borderRadius: 100, background: `linear-gradient(135deg, ${GREEN}, ${GREEN_DK})`, color: '#fff', textDecoration: 'none', fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 15, boxShadow: '0 4px 24px rgba(68,178,76,0.4)' }}>
              Apply for Franchise <ArrowRight size={16} />
            </motion.a>
            <motion.a href="/pricing" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 28px', borderRadius: 100, background: 'transparent', color: 'rgba(255,255,255,0.8)', border: '1.5px solid rgba(255,255,255,0.25)', textDecoration: 'none', fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 15 }}>
              View Pricing Models
            </motion.a>
          </div>
        </motion.div>
